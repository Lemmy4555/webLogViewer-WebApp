...
export class DbHelper {
   ...
   */
  public updateFile(file: FileCompleteInterface) {
    var request = this.dbService.getFile(file.path);
    request.onsuccess = (event: FileInterface) => {
      var result = request.result;
      if(!result) {
        this.logger.warn("Non e stato trovato nessun file salvato con il path: %s", file.path);
        return;
      }
      var fileRead = FileCompleteModel.buildFromJson(result);
   ...
   */
  public addFile(file: FileCompleteInterface) {
